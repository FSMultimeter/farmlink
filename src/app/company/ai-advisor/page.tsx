...
export default function CompanyAIAdvisorPage() {
  const [messages, setMessages] = useState<Message[]>([
    { role: "ai", text: "Hello! Ask me about crop sourcing, fair pricing, negotiating with farmers, or how to use FarmLink." },
  ]);
  ...
  const handleSend = async () => {
    ...
    try {
      const res = await fetch("/api/advisor/company-advisor", {
        method: "POST",
    ...
    <div className="min-h-screen bg-green-50 flex flex-col">
      <div className="max-w-2xl mx-auto w-full flex flex-col flex-1 px-4 py-6">
        <div className="flex justify-between items-center mb-4">
          <h1 className="text-2xl font-bold text-green-800">AI Procurement Advisor</h1>
            ...
            <div key={i} className={`flex ${msg.role === "user" ? "justify-end" : "justify-start"}`}>
              <div
                className={`max-w-[80%] px-4 py-2 rounded-2xl text-sm ${
                  msg.role === "user" ? "bg-green-600 text-white" : "bg-green-100 text-green-900"
                }`}
              >
                {msg.text}
              </div>
        ...
        <div className="flex gap-2">
          <input
            ...
            placeholder="Ask about sourcing, pricing, negotiating..."
